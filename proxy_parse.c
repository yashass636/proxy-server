...
static char* FindCol(char* str, char ch)
{
    int len = strlen(str);
    ...
    int buflen)
{
    ...
    /* Parse request line */
    parse->method = strtok_s(parse->buf, " ", &saveptr);
    ...
    if (parse->path == NULL) {          // replace empty abs_path with "/"
        ...
        strncpy_s(parse->path, sizeof(parse->path), root_abs_path, rlen + 1);
    }
        ...
        strncpy_s(parse->path, sizeof(parse->path), root_abs_path, rlen);
        char* new_buf = parse->path + rlen;
        strncpy_s(new_buf, plen + 1, tmp_path, plen + 1);
    }
